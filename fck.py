...
threshold = 0.5  # Define a threshold for migration
...
for compute_node, file_name in compute_nodes.items():
    ...
    if median_predicted_value <= threshold:
        ...
        script_path = os.path.join(script_dir, "fetch_instance.sh")
        command = f"bash {script_path} {compute_node} instances.txt"
        print(f"Executing command: {command}")
        result = os.system(command)

        # Verify if `instances.txt` was created successfully
        instances_file_path = os.path.join(script_dir, "instances.txt")
        if os.path.exists(instances_file_path):
            print(f"File instances.txt successfully created for {compute_node}.")
        else:
            print(f"Error: Failed to create instances.txt for {compute_node}.")
            continue  # Move to the next compute node
